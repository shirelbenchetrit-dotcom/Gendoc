// Envoi en batch des convocations d'oraux blancs
// Usage : node scripts/send-convocations-batch.mjs
...
const COMMON = {
  ...
  zoomLink: 'https://us02web.zoom.us/j/88913149340?pwd=Z5Qd41fPm0cQ7ilEwQb1D6vKTckrRa.1', // Diplomasante1
  formulaireSujet: 'https://forms.gle/5pDtPWepFKDJBiNh9',
}

// === Liste des 16 élèves (samedi 16/05/2026, planning Mme Benramdane) ===
const students = [
  ...
  { prenom: 'Hawa',     email: 'chawa652@gmail.com',            h1: '17:30', h2: '18:00', sujet: '' },
            ...
            <tr><td style="padding:20px 22px;">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" style="font-size:13px;">
                <tr>
                  ...
                  <td style="color:#0f172a;padding:5px 0;font-weight:600;">${COMMON.jour} ${COMMON.date}</td>
                </tr>
                <tr>
                  <td style="color:#64748b;padding:5px 0;vertical-align:top;">Horaire</td>
                  <td style="color:#0f172a;padding:5px 0;font-weight:600;">${s.h1} &ndash; ${s.h2}</td>
                </tr>
                <tr>
                  <td style="color:#64748b;padding:5px 0;vertical-align:top;">Professeur</td>
                  <td style="color:#0f172a;padding:5px 0;">${COMMON.professeur}</td>
                </tr>
                <tr>
                  <td style="color:#64748b;padding:5px 0;vertical-align:top;">Lien Zoom</td>
                  <td style="padding:5px 0;">
                    <a href="${COMMON.zoomLink}" style="color:#059669;font-weight:600;text-decoration:none;">▶ Rejoindre la session</a>
                  </td>
                </tr>
                <tr>
...
async function sendOne(student) {
  const payload = {
    ...
    to: [{ email: student.email, name: student.prenom }],
    subject: `Convocation oral blanc — ${COMMON.matiere} — ${COMMON.jour} ${COMMON.date}`,
    htmlContent: buildHtml(student),
  }
...
console.log(`→ Envoi de ${students.length} convocations...\n`)
...
for (const s of students) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      results.push({ student: s, ok: false, error: `${r.status} ${r.body}` })
    }
  } catch (err) {
    console.log(`✖ ${err.message}`)
    results.push({ student: s, ok: false, error: err.message })
  }
  // Petit délai pour éviter le rate limit Brevo
...
if (ko.length) {
  console.log(`\nÉchecs :`)
  ko.forEach(r => console.log(`  - ${r.student.prenom} (${r.student.email}) : ${r.error}`))
}
